...
indir='/data/home/Plutea_mangroves/diamond_speciesID'
files_pattern='(.*)(?=\\.m8$)'
design_file='/data/home/Plutea_mangroves/scripts/design/metadata_diamond_Plut.csv'

design_factor='location'
design_factor_levels=c("reef","mangrove")
...
clades_name=c('Symbiodinium','Coral')
outidx='/data/home/Plutea_mangroves/diamond_speciesID/heatmap'
...
names(t_design1) = c('sample','location')
t_design1$location = factor(t_design1$location,levels=design_factor_levels)
...
for(k in 1:length(clades_name)){
    ...
    hm1 = ggplot(data = mlt2, mapping = aes(x = variable, y = species, fill = value)) +
        ...
        theme(axis.text.x=element_text(angle = 45)) + facet_grid(cols = vars(location),scale='free') +
